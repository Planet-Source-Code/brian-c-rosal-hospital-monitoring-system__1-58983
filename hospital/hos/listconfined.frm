...
End If
sql = "select plastname,pfirstname,pmi,page,padiagnose,patientno,idno,datec,timein from Patient where TRIM(idno) LIKE  TRIM('" & tb & "') and format(dateD,'mm/dd/yyyy') = '" & testdate & "' AND ISNULL(IDNO)=FALSE order by plastname,pfirstname,pmi;"
           Set rs = DBMain.OpenRecordset(sql)
              If rs.RecordCount < 0 Or rs.RecordCount = 0 Then
                       sql = "select plastname,pfirstname,pmi,page,padiagnose,patientno,idno,datec,timein from Patient where format(dateD,'mm/dd/yyyy') = '" & testdate & "' AND ISNULL(IDNO)=FALSE order by plastname,pfirstname,pmi;"
                       Set rs = DBMain.OpenRecordset(sql)
